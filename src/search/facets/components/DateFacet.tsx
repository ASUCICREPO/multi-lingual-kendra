...
export class DateFacet extends React.Component<Props, State> {
  ...
  getFullSliderRange = (): [number, number] => {
    ...
    if (selections) {
      return [
        Math.min(availableDateFacet.minYear - 1, selections.min.get("year")),
        Math.max(availableDateFacet.maxYear, selections.max.get("year")),
      ];
    } else {
      return [availableDateFacet.minYear - 1, availableDateFacet.maxYear];
    }
  };
  ...
  getFullHeuristicChartRange = (): [number, number] => {
    ...
    if (selections) {
      return [
        Math.min(availableDateFacet.minYear, selections.min.get("year") + 1),
        Math.max(availableDateFacet.maxYear, selections.max.get("year")),
      ];
    } else {
      return [availableDateFacet.minYear, availableDateFacet.maxYear];
    }
  };
  ...
  getDisplaySelectionRange = (): [number, number] => {
    ...
    if (slider.value) {
      ...
      return [slider.value[0] + 1, slider.value[1]];
    } else {
  ...
  render() {
    const { attributeName, disableAdd, valueCountPairs, selections } =
      this.props;
    ...
    return (
      <div className="facet">
        ...
        <div className="facet-date-container">
          ...
          />
          <div>
            <div className="facet-date-label-left">
              {displaySelectionRange[0]}
            </div>
            <div className="facet-date-label-right">
