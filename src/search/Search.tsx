//  "indexId": "c0900ac7-9d8a-435f-bdb6-ba408cb89a96"
import Kendra, { QueryRequest, QueryResultItemList } from "aws-sdk/clients/kendra";
  ...
  listDataSources = async (
    ...
    try {
      ...
      while (listDsResponse?.$response.hasNextPage()) {
        const nextPage: Request<Kendra.ListDataSourcesResponse, AWSError> | null = listDsResponse.$response.nextPage();
        if (nextPage) {
  ...
  private getResultsHelper = async (
    ...
    try {
      ...
      if (results && results.ResultItems) {
        ...
        //   {
        //     result.DocumentTitle.Text= await textTranslate('en', detectedlang , result.DocumentTitle?.Text)
              ...
              {this.state.searchResults.TotalNumberOfResults !== 0 && (
                <div>
                  <ResultPanel
                    ...
                    resultLanguage={this.state.queryTextLanguage}
...
async function translateResults(detectedlanguage: string, resultsarray: Kendra.QueryResultItemList) {
  ...
  const translatedResults = await Promise.all(resultsarray.map(async (result) => {
    ...
    if (result.DocumentExcerpt?.Text) {
      result.DocumentExcerpt.Text = await textTranslate('en', detectedlanguage, result.DocumentExcerpt.Text);
    }

    if (result.DocumentTitle?.Highlights) {
      result.DocumentTitle.Highlights = [];
    }
    if (result.DocumentExcerpt?.Highlights) {
      result.DocumentExcerpt.Highlights = [];
    }
    if(result.AdditionalAttributes?.[0]?.Value?.TextWithHighlightsValue?.Highlights) {
      result.AdditionalAttributes[0].Value.TextWithHighlightsValue.Highlights = [];
    }

    if(result.AdditionalAttributes?.[0]?.Value?.TextWithHighlightsValue?.Text) {
      result.AdditionalAttributes[0].Value.TextWithHighlightsValue.Text = await textTranslate('en', detectedlanguage, result.AdditionalAttributes[0].Value.TextWithHighlightsValue.Text);
    }
